# STACK SITE

## Contexto

- Stacks definem os conjuntos de tecnologia usadas para um objetivo comum.
- Stacks não são certas ou erradas, mas têm prós e contras, e funcionam melhor ou pior dependendo do contexto.
- Desenvolvimento de sites é baseado em tecnologias web como HTML, CSS, JavaScript, rodando em navegadores através de HTTP.
- Uma stack de site inclui apenas as tecnologias necessárias

## Decisão

- Usar uma stack moderna para o desenvolvimento de sites, balanceando velocidade, qualidade e performance.

## Consequências

- Prós:
  - Alta velocidade de desenvolvimento, incluindo servidor de desenvolvimento, rotas baseadas em sistema de arquivos, renderização de Markdown.
  - Alta qualidade de desenvolvimento, incluindo testes E2E, SSR e otimização de imagens.
- Contras:
  - Baixa performance de processamento (Node, Playwright) e armazenamento (npm, Astro).
  - Limitado para páginas e APIs, excluindo processamento assíncrono com maior volume de dados e latência.

## Implementação

Tecnologias:

- Usar TypeScript como linguagem de programação.
- Usar Node como runtime.
- Usar npm como gerenciador de dependências, pacotes e scripts.
- Usar Playwright como ferramenta de testes E2E.
- Usar Astro como framework de desenvolvimento.
- Usar Tailwind como ferramenta de CSS.
